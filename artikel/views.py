...
def artikel_update(request, id_artikel):
    ...
    try:
        ...
        messages.error(request, "Halaman tidak di temukan")
...
def admin_kategori_update(request, id_kategori):
    template_name = "dashboard/admin/kategori_forms.html"
    kategori_obj = Kategori.objects.get(id=id_kategori)  # ✅ pakai nama lain
    
    if request.method == "POST":
        forms = KategoriForms(request.POST, instance=kategori_obj)
        if forms.is_valid():
            pub = forms.save(commit=False)
            pub.created_by = request.user
            pub.save()
    ...
    forms = KategoriForms(instance=kategori_obj)
    context = {
        "forms": forms
    }
    return render(request, template_name, context)
...
def admin_artikel_update(request, id_artikel):
    template_name = "dashboard/admin/artikel_forms.html"
    artikel = ArtikelBlog.objects.get(id=id_artikel)  # ✅ pakai nama lain
...
def admin_management_user_list(request):
    ...
    return render(request, template_name, contextt)
